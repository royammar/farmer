        ...
        return (
            <div className={`nav-bar container flex ${toggle}`}>
                ...
                <div className={`right-side-nav flex align-center ${toggle}`}>
                 ...
                 {loggedInUser? <div className="avatar-circle"><span className="avatar" onClick={handleClick}>{loggedInUser.username.substring(0,1)} {showMenu&&<NavBarMenu loggedInUser={loggedInUser} handleLogOut={handleLogOut} createShop={createShop}></NavBarMenu>}</span></div>:<Link className="flex nav " to="/login" > <img className="nav-icon" src={User}></img></Link>}
                    <Link className="flex nav" to="/wishlist" ><img className="nav-icon"  src={Heart}/><span className='wishlist-amount flex align-center justify-center'>{wishlist.length}</span></Link>
                    <Link className="flex nav" to="/cart" ><img  className="nav-icon" src={Cart}/><span className='cart-amount flex align-center justify-center'>{cart.length}</span></Link>
                </div>
            </div>
        )
    }
